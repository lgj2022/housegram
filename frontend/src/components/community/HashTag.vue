  ...
  <div class="comp_hashtag" @click="setHashtags" ref="group">
    ...
    <div class="tags" v-if="!helpVisible">
      ...
      <!-- <template>
        <span
          class="tag"
          v-for="(row, index) in articleItem.tagInfos"
          :key="index"
...
export default {
  ...
  async created() {
    await bus.$on("request:HashTags", async () => {
      bus.$emit("response:HashTags", this.tags);
      this.tags = [];
    });
    // if (this.articleItem.tagInfos) {
    //   console.log(this.articleItem.tagInfos);
    // }
  },
  computed: {
