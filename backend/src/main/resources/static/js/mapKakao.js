...
encoding key : FBcNOTDKCffi884b%2FbAJHUswz0w5sQ92oQ9hhwGXtVm0khubm14kdtIWrP7VRKkJNal6Nh0w8hROdDLQbbH3QQ%3D%3D
decoding key : FBcNOTDKCffi884b/bAJHUswz0w5sQ92oQ9hhwGXtVm0khubm14kdtIWrP7VRKkJNal6Nh0w8hROdDLQbbH3QQ==
*/
      ...
      .then((resList) => {
        ...
        for (let i = 0; i < resList.length; i++) {
          ...
          if (resList[i]["법정동"] == " " + dong) {
            ...
            // 서울특별시 종로구 청운동 9
            let mapAddress = `${sido} ${gugun}${townCode} ${townNum}`;
        ...
        if (searchCnt == 0) {
          ...
          $("#info").parent().css("display", "none");
          //alert('검색 결과가 없습니다.');
          return;
        } else {
          if ($("#info").parent().css("display") == "none") {
            $(".col").attr("class", "col-8");
            $("#info").parent().css("display", "block");
          }
        }
        aptInfoSetting();
      });
...
function aptInfoSetting() {
  let aptInfo = document.querySelectorAll(".list-group-item");
  for (let i = 0; i < aptInfo.length; i++) {
    aptInfo[i].addEventListener("click", (e) => {
      e.preventDefault();
      console.log(aptInfo[i].dataset.cost.trim());
      console.log(aptInfo[i].dataset.date);
      console.log(aptInfo[i].dataset.aptname);
    });
  }
}